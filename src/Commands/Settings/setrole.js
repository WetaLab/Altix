...
module.exports = {
  name: "setrole",
  description:
    "Sets the role that the bot will give to the user when they verify.",
  ephemeral: true,
  options: [
    {
      name: "role",
  ...
  async execute(client, interaction) {
    ...
    client.database
      .prepare(`UPDATE verifysettings SET role = ? WHERE guildid = ?`)
      .run(role.name, interaction.guild.id.toString());
    let Response = new EmbedBuilder()
      .setColor(0xffffff)
      .setDescription(`<a:success:884527566688509982> | The verification role has been set to \`${role.name}\``);
    return interaction.followUp({
      embeds: [Response],
      ephemeral: true,
    });
